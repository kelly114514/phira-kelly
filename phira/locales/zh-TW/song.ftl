...
custom-judgement-notice = 已啟用 CUSTOM P/G/B { $perfect }/{ $good }/{ $bad }ms，本局成績僅儲存在本機且絕不會上傳。
multiplayer-default-judgement = 多人遊戲使用原版 80/160/220ms 判定。
custom-active-chip = CUSTOM · P/G/B { $perfect }/{ $good }/{ $bad } · 僅本機
custom-best-source = 自訂最佳 · 達成於 { $perfect }/{ $good }/{ $bad }ms
custom-best-empty = 自訂最佳 · 尚無紀錄
custom-local-rank = 僅本機 · 可查看標準排行榜
