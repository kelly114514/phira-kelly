...
item-speed = 音符流速
item-note-size = 音符大小
item-judgement-windows = 判定範圍
item-judgement-windows-sub = 調整累計判定邊界；自訂成績僅儲存在本機
judgement-original-summary = 原版 · { $perfect } / { $good } / { $bad } ms
judgement-custom-summary = CUSTOM · { $perfect } / { $good } / { $bad } ms
judgement-page-label = 判定範圍
judgement-original-title = 原版判定
judgement-original-mode-sub = 使用原有遲按補償與距離修正，可正常上傳雲端成績
judgement-custom-title = CUSTOM · 嚴格對稱判定
judgement-custom-mode-sub = 僅按絕對時間偏差使用對稱窗口，成績只儲存在本機
judgement-tier-inactive = 未啟用
judgement-local-only-notice = 自訂判定成績僅儲存為自訂本機最佳，不參與雲端排名；多人遊戲一律使用原版判定。
judgement-reset = 一鍵重設為預設值
judgement-input-offset = 輸入時間補償
judgement-input-offset-sub = 僅修正觸控判定時間；聲音偏移仍會獨立生效
