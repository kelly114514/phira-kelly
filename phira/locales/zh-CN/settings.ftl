...
item-speed = 速度
item-note-size = 音符大小
item-judgement-windows = 判定范围
item-judgement-windows-sub = 调整累计判定边界；自定义成绩仅保存在本地
judgement-original-summary = 原版 · { $perfect } / { $good } / { $bad } ms
judgement-custom-summary = CUSTOM · { $perfect } / { $good } / { $bad } ms
judgement-page-label = 判定范围
judgement-original-title = 原版判定
judgement-original-mode-sub = 使用原有迟按补偿与距离修正，可正常上传云端成绩
judgement-custom-title = CUSTOM · 严格对称判定
judgement-custom-mode-sub = 仅按绝对时间偏差使用对称窗口，成绩只保存在本地
judgement-tier-inactive = 未启用
judgement-local-only-notice = 自定义判定成绩仅保存为自定义本地最佳，不参与云端排名；多人游戏始终使用原版判定。
judgement-reset = 一键重置为默认
judgement-input-offset = 输入时间补偿
judgement-input-offset-sub = 仅修正触摸判定时间；声音偏移仍然独立生效
