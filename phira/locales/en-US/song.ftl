...
custom-judgement-notice = CUSTOM P/G/B { $perfect }/{ $good }/{ $bad }ms is enabled. This result is saved locally only and will never be uploaded.
multiplayer-default-judgement = Multiplayer uses the original 80/160/220ms judgement.
custom-active-chip = CUSTOM · P/G/B { $perfect }/{ $good }/{ $bad } · LOCAL
custom-best-source = CUSTOM BEST · achieved at { $perfect }/{ $good }/{ $bad }ms
custom-best-empty = CUSTOM BEST · no record yet
custom-local-rank = Local only · view standard leaderboard
