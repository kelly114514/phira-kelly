...
item-judgement-windows = Judgement Windows
item-judgement-windows-sub = Customize cumulative timing boundaries. Custom results stay local.
judgement-original-summary = Original · { $perfect } / { $good } / { $bad } ms
judgement-custom-summary = CUSTOM · { $perfect } / { $good } / { $bad } ms
judgement-page-label = JUDGEMENT WINDOWS
judgement-original-title = Original Judgement
judgement-original-mode-sub = Uses the original timing compensation and distance adjustment. Cloud upload remains available.
judgement-custom-title = CUSTOM · STRICT SYMMETRIC
judgement-custom-mode-sub = Uses absolute timing error with symmetric windows. Results are local only.
judgement-tier-inactive = inactive
judgement-local-only-notice = Custom judgement results are saved only as the custom local best and never enter cloud rankings. Multiplayer always uses the original judgement.
judgement-reset = Reset to Default
judgement-input-offset = Input Timing Compensation
judgement-input-offset-sub = Adjusts timestamped touch judgement only; audio offset remains independent.
